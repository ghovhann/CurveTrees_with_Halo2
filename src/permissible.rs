...
impl<F:Field> PrmsChip<F> {
    ...
    fn configure(meta: &mut ConstraintSystem<F>) -> PrmsConfig {
        ...
        meta.create_gate("is point on curve", |meta|{
            ...
            let on_curve = b.square() - a.clone().square() * a - Expression::Constant(pallas::Affine::b());

            vec![s*on_curve]
        });

        PrmsConfig { advice: ([col_a, col_b, col_c]), selector: (selector_1)}
    }

    fn assign(&self, mut layouter: impl Layouter<F>, x: &Value<F>, y: &Value<F>, y_sqrt: &Value<F>) 
