...
impl<F:Field> Circuit<F> for MyCircuit<F> {
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
        ...
        chip.assign(layouter.namespace(|| "select"), &self.commits, &self.witness, &self.w_sqrt, self.k)?;
...
impl<F:Field> MyChip<F> {
    ...
    fn configure(meta: &mut ConstraintSystem<F>) -> MyConfig {
        ...
        MyConfig { advice: ([col_a, col_b, col_c, col_d]), selector: ([selector_1, selector_2, selector_3])}
    }

    fn assign(&self, mut layouter: impl Layouter<F>, commit: &Vec<Value<F>>, witness: &Value<F>, w_sqrt: &Value<F>, num_rows: usize) 
...
fn main()
{
    ...
    let end_time = Instant::now();
    let elapsed_time = end_time.duration_since(start_time);
    println!("Elapsed batch verifier time: {:?}ms", elapsed_time.as_millis());
